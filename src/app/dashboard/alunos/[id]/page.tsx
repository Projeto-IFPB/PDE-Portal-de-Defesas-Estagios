...
export default async function DetalhesAlunoPage({
  ...
  const fotoUrl = await obterUrlPublicaFotoPerfil(
    caminhoFoto || "sem_foto_perfil.jpg"
  );

  const agora = new Date();
  const dataInicio = estagio.data_de_inicio
    ? new Date(estagio.data_de_inicio)
    : null;
  const dataFim = estagio.previsao_data_fim
    ? new Date(estagio.previsao_data_fim)
    : null;
  ...
  if (!isPendente && dataInicio && dataFim) {
    ...
    if (tempoTotal > 0) {
      progressoEstagio = Math.max(
        0,
        Math.min(100, Math.round((tempoDecorrido / tempoTotal) * 100))
      );
    }
  }

  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto bg-gray-50 min-h-screen dark:bg-slate-950">
      <div className="flex items-center text-sm text-gray-500 mb-4 select-none dark:text-slate-400">
        <Link
          href="/dashboard/alunos"
          className="hover:text-gray-700 cursor-pointer transition-colors dark:hover:text-slate-200"
        >
          Alunos
        </Link>
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-6 dark:text-slate-50">
        Detalhes do Aluno
      </h1>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col items-center text-center dark:bg-slate-900 dark:border-slate-800">
            ...
            <h2 className="text-xl font-bold text-gray-900 line-clamp-2 px-2 dark:text-slate-50">
              {usuario.Nome_Completo}
            </h2>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 dark:bg-slate-900 dark:border-slate-800">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-5 dark:text-slate-50">
              Informações Pessoais
            </h3>
            <div className="space-y-5">
              <div className="flex gap-3 items-start">
                <div className="p-2 bg-blue-50 rounded-lg text-[#0052cc] shrink-0">
                  <Mail className="w-4 h-4" />
                </div>
                <div className="min-w-0 flex-1">
                  <p
                    className="text-xs text-gray-500 font-medium dark:text-slate-50
"
                  >
                    Email Corporativo
                  </p>
                  <p className="text-sm font-semibold text-gray-900 truncate dark:text-slate-400">
...
"
                  ...
                  <p className="text-sm font-semibold text-gray-900 truncate dark:text-slate-400">
                    {estagio.curso || "Engenharia de Software"}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-[#0052cc] rounded-2xl shadow-sm p-6 text-white relative overflow-hidden">
            ...
            <p className="text-blue-100 text-sm mb-6">
              Métricas correspondentes ao cumprimento das etapas obrigatórias.
            </p>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 dark:bg-slate-900">
            <div className="flex justify-between items-center mb-6 gap-4">
              <div className="flex items-center gap-2 min-w-0">
                <Briefcase className="w-5 h-5 text-[#0052cc] shrink-0" />
                <h3 className="text-lg font-semibold text-gray-900 truncate dark:text-slate-50">
                  Detalhes do Estágio
                </h3>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-6">
              <div className="border-l-2 border-gray-100 pl-4">
                <p className="text-xs text-gray-400 font-bold uppercase tracking-wider mb-1 dark:text-slate-400">
                  Empresa contratante
                </p>
                <p className="text-base font-semibold text-gray-900 dark:text-slate-50">
                  {estagio.empresa || "Não informada"}
                </p>
              </div>
              <div className="border-l-2 border-gray-100 pl-4">
                <p className="text-xs text-gray-400 font-bold uppercase tracking-wider mb-1 dark:text-slate-400">
                ...
                <p className="text-base font-semibold text-gray-900 dark:text-slate-50">
                  {orientador.Nome_Completo || "Orientador Acadêmico"}
                </p>
              </div>
              <div className="border-l-2 border-gray-100 pl-4">
                <p className="text-xs text-gray-400 font-bold uppercase tracking-wider mb-1 dark:text-slate-400">
                ...
                <p className="text-base font-semibold text-gray-900 dark:text-slate-50">
                  {isPendente ? (
                    ...
                    new Date(estagio.data_de_inicio).toLocaleDateString(
                      "pt-BR",
                      { timeZone: "UTC" }
                    )
                  ) : (
              ...
              <div className="border-l-2 border-gray-100 pl-4">
                <p className="text-xs text-gray-400 font-bold uppercase tracking-wider mb-1 dark:text-slate-400">
                  Regime Horário Semanal
                </p>
                <p className="text-base font-semibold text-gray-900 dark:text-slate-50">
                  {estagio.carga_horaria
                    ? `${estagio.carga_horaria} horas semanais`
                    : "Não informada"}
                </p>
              </div>
            </div>
