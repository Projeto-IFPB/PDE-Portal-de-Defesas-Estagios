...
export function CardBancaDefesa({data, status, titulo, horario, local, banca}: InterfaceCardBancaDefesa) {
    ...
    return (
        <>
            <div className="bg-white p-4 rounded-xl dark:bg-slate-900">
                <div className="flex justify-between mb-5">
                    <div className="flex text-blue-700 font-semibold items-center">
                    <Calendar className="mr-2 w-6 h-6 shrink-0" />
                    <p className="text-sm dark:text-slate-400">{data}</p>
                    </div>
                    <div className={`flex rounded-full px-3 items-center ${styleStatus[status]}`}><p className="text-[0.8rem]">{status}</p></div>
                    
                </div>

                <h1 className="text-xl font-semibold mb-2 dark:text-slate-50">{titulo}</h1>

                <div className="flex items-center text-sm text-slate-600 mb-4 dark:text-slate-400">
                ...
                <div className="bg-slate-100 p-3 rounded-md space-y-3 dark:bg-slate-900">
                    <p className="uppercase font-semibold text-[0.8rem] text-slate-600 dark:text-slate-50">Banca Examinadora</p>
                    {banca.map((membro, i) => (
                    <div key={i} className="flex items-center">
                    <div className="flex bg-slate-300 p-[0.4rem] rounded-full mr-2 items-center"><UserRound className="w-4 h-4 shrink-0" /></div>
                    <p className="text-slate-700 dark:text-slate-400">{membro.nome}</p>
                    </div>
                    ))}
                </div>
            </div>
    ...
    return (
        ...
        <div className="bg-white py-10 flex flex-col text-center items-center dark:bg-slate-900">
            ...
            <div className="max-w-80 text-center">
                <h1 className="text-xl font-semibold mb-3 dark:text-slate-50">Nenhuma banca agendada</h1>
                <p className="text-slate-500 dark:text-slate-400">Você não possui banca de defesa agendada para este semestre. Novas bancas aparecerão aqui quando forem confirmadas.</p>
            </div>
        </div>
        </>
    )
