...
async function carregar_pagina() {
  ...
  const usuario_logado = lista_usuarios.find(
    (user) => user["email"] === localStorage.getItem("EmailUsuario")
  );
  const meu_id = usuario_logado ? usuario_logado["id"] : null;

  if (meu_id) {
    const qtd_alunos = await dados_estatisticos_orientador(
      meu_id,
      "total_alunos_orientador"
    );
    const qtd_estagios_ativos = await dados_estatisticos_orientador(
      meu_id,
      "estagios_ativos_orientador"
    );
    const qtd_pendencias = await dados_estatisticos_orientador(
      meu_id,
      "estagios_pendentes_orientador"
    );

    document.querySelector(".card-alunos .card-valor").textContent = qtd_alunos;
    document.querySelector(".card-estagios-ativos .card-valor").textContent =
      qtd_estagios_ativos;
    document.querySelector(".card-pendencias .card-valor").textContent =
      qtd_pendencias;
  }
}
